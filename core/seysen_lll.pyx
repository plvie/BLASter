...
def block_svp(int beta,
    ...
    cdef FT[:, ::1] R_sub = np.empty(shape=(num_blocks, beta**2), dtype=NP_FT)
    cdef ZZ[:, ::1] U_sub = np.empty(shape=(num_blocks, beta**2), dtype=NP_ZZ)
    ...
    for block_id in prange(num_blocks, nogil=True):
        ...
        svp_reduce(w, &R_sub[block_id, 0], &U_sub[block_id, 0], delta)

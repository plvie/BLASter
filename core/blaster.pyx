...
def block_lll(
    ...
    cdef:
        ...
        FT[:, ::1] R_sub = cnp.empty(shape=(num_blocks, block_size**2), dtype=NP_FT)
        ZZ[:, ::1] U_sub = cnp.empty(shape=(num_blocks, block_size**2), dtype=NP_ZZ)
...
@cython.wraparound(False)
def block_lll_gpu(
        ccp.ndarray[FT, ndim=2] R, ccp.ndarray[ZZ, ndim=2] B_red, ccp.ndarray[ZZ, ndim=2] U,
        FT delta, int offset, int block_size) -> None:
    global debug_size_reduction
    ...
    cdef:
        Py_ssize_t n = R.shape[0]
        int i, j, w, num_blocks = int((n - offset + block_size - 1) / block_size), block_id
        FT[:, ::1] R_sub = ccp.empty(shape=(num_blocks, block_size**2), dtype=NP_FT)
        ZZ[:, ::1] U_sub = ccp.empty(shape=(num_blocks, block_size**2), dtype=NP_ZZ)
